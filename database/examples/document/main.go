...
func run() error {
	...
	changes, err := db.Driver.(database.Watcher).Watch(watchCtx, books, database.WatchOptions{})
	if err != nil {
	...
	recent, err := db.List(ctx, books, database.ListOptions{
	...
	_, err = db.List(ctx, books, database.ListOptions{Filter: "title LIKE Dune"})
	log.Printf("unsupported filter: %v", err)

	// --- bulk ---
	batcher := db.Driver.(database.Batcher)
...
func kind(k database.ChangeKind) string {
	switch k {
	case database.ChangeCreated:
		return "a create"
	case database.ChangeUpdated:
		return "an update"
	case database.ChangeDeleted:
		return "a delete"
	default:
		return "a change"
	}
