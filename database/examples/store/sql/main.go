...
func run() error {
	...
	// Step two and three: declare the provider, then select a store. Nothing
	...
	log.Printf("duplicate title: %v", errors.Is(err, store.ErrAlreadyExists))
	...
	stored, err := db.Count(ctx, books, store.ListOptions{})
	if err != nil {
		return err
	}
	...
	typed, err := store.For[*dynamicpb.Message](db, books)
	if err != nil {
		return err
	}
	page, _, total, err := typed.List(ctx, store.ListOptions{PageSize: 10})
	if err != nil {
		return err
	}
	...
	// pretending.
	_, err = db.Graph.Neighbors(ctx, store.Ref{Resource: "Book", Key: "books/dune"},
		store.TraverseOptions{MaxDepth: 1})
