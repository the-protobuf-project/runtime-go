...
func run() error {
	...
	backing, err := orm.NewProvider(sqlClient).SetDatabase(ctx, "")
	if err != nil {
		return err
	}
	...
	// timeout.
	counted := &countingDriver{Driver: backing.Driver}
	...
	db := cached.Wrap(&store.DB{
		Driver:  counted,
		Tx:      backing.Tx,
		Schema:  backing.Schema,
		Graph:   backing.Graph,
		Series:  backing.Series,
...
func (c *countingDriver) Get(ctx context.Context, res *store.Resource, key string) (proto.Message, error) {
	c.gets.Add(1)
