...
func run() error {
	...
	for _, res := range []*database.Resource{books, authors, wrote} {
	...
	herbert := database.Ref{Resource: "Author", Key: "authors/herbert"}
	...
	err = db.Tx.Run(ctx, func(tx *database.DB) error {
		for _, id := range []string{"books/dune", "books/messiah"} {
			if _, err = tx.Graph.Connect(ctx, wrote,
				herbert, database.Ref{Resource: "Book", Key: id},
	...
	edges, err := db.Graph.Neighbors(ctx, herbert, database.TraverseOptions{
		Types: []string{"Wrote"}, Direction: database.Outbound, WithProps: true,
	})
	if err != nil {
		return err
	}
	...
	titles, err := database.Resolve[*dynamicpb.Message](ctx, db, model.Registry(), database.Ends(edges))
	if err != nil {
		return err
	}
	...
	back, err := db.Graph.Neighbors(ctx, database.Ref{Resource: "Book", Key: "books/dune"},
		database.TraverseOptions{Types: []string{"Wrote"}, Direction: database.Inbound})
	if err != nil {
		return err
	}
	...
	_, err = db.Graph.Connect(ctx, wrote, herbert,
		database.Ref{Resource: "Book", Key: "books/ghost"}, nil)
	log.Printf("edge to a missing record: %v", errors.Is(err, database.ErrNotFound))

	return nil
}

func mustGet(ctx context.Context, db *database.DB, res *database.Resource, key string) proto.Message {
