...
func run() error {
	...
	day2, err := db.Series.Range(ctx, readings, database.RangeOptions{
	...
	buckets, err := db.Series.Aggregate(ctx, readings, database.AggregateOptions{
		TimeColumn: "observed_at",
		Start:      base,
		End:        base.Add(48 * time.Hour),
		Every:      12 * time.Hour,
		...
		Reduce: []database.Reduction{
			...
			{Func: database.Max, Column: "celsius", As: "peak"},
		},
	})
	if err != nil {
		return err
	...
	// a question at all.
	_, err = db.Series.Aggregate(ctx, readings, database.AggregateOptions{
		TimeColumn: "observed_at", Every: time.Hour,
		Reduce: []database.Reduction{{Func: database.Avg, Column: "sensor"}},
	})
