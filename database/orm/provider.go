...
// [database.ErrAlreadyExists] and [database.ErrNotFound]. Without it those come back
...
// result is qualified with it, overriding [database.Resource.Schema] — which is
...
// with. Nothing is derived either way, so [database.DB.Close] is a no-op: the
// connection pool is the one you handed in.
func (p *Provider) SetDatabase(ctx context.Context, name string) (*database.DB, error) {
	if p.db == nil {
		return nil, fmt.Errorf("%w: orm provider has no *gorm.DB", database.ErrBadConfig)
	}
	...
	return database.Build(&Driver{db: p.db, schema: name}, "gorm", name, nil), nil
}
