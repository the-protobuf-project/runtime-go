// Package redis implements the backend-agnostic database.Driver over Redis: durable
// records addressed by a [database.Resource]'s primary key, stored as proto wire
...
//
// It implements [database.Migrator] and [database.Batcher], and not
// [database.Transactional] — Redis has no rollback across the several keys one
// write touches, so the capability is absent rather than approximated, and
// [database.DB.Tx] reports [database.ErrUnimplemented] naming the backend.
//
...
// OrderBy are ignored, as [database.ListOptions] permits. That is not a gap to fill
