...
//
// The name becomes a key segment rather than a Redis numbered store. Two
...
func (p *Provider) SetDatabase(ctx context.Context, name string) (*store.DB, error) {
	if p.rdb == nil {
		return nil, fmt.Errorf("%w: redis provider has no client", store.ErrBadConfig)
	}
	...
	return store.Build(d, "redis", name, nil), nil
}
