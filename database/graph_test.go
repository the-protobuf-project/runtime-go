...
func TestTransactionBodySeesEveryCapability(t *testing.T) {
	db := database.Build(bare{}, "fake", "test", nil)
...
func TestCapabilityFieldsAreNeverNil(t *testing.T) {
	...
	if _, err := db.Graph.Connect(ctx, nil, database.Ref{}, database.Ref{}, nil); !errors.Is(err, database.ErrUnimplemented) {
		t.Errorf("Graph.Connect = %v, want ErrUnimplemented", err)
	}
	if _, err := db.Graph.Neighbors(ctx, database.Ref{}, database.TraverseOptions{}); !errors.Is(err, database.ErrUnimplemented) {
		t.Errorf("Graph.Neighbors = %v, want ErrUnimplemented", err)
	}
	if _, err := db.Graph.Traverse(ctx, database.Ref{}, database.TraverseOptions{}); !errors.Is(err, database.ErrUnimplemented) {
		t.Errorf("Graph.Traverse = %v, want ErrUnimplemented", err)
	}
	if err := db.Graph.Disconnect(ctx, nil, "x"); !errors.Is(err, database.ErrUnimplemented) {
...
func TestGraphRefusalNamesTheBackend(t *testing.T) {
	db := database.Build(bare{}, "postgres", "", nil)
	_, err := db.Graph.Neighbors(context.Background(), database.Ref{}, database.TraverseOptions{})
...
func TestPathReadsItsOwnRoute(t *testing.T) {
	...
	origin := database.Path{Vertices: []database.Ref{{Resource: "User", Key: "a"}}}
...
func TestEndsAndStarts(t *testing.T) {
	edges := []database.Edge{
		{From: database.Ref{Resource: "User", Key: "a"}, To: database.Ref{Resource: "User", Key: "b"}},
		{From: database.Ref{Resource: "User", Key: "a"}, To: database.Ref{Resource: "User", Key: "c"}},
	}
	ends := database.Ends(edges)
	if len(ends) != 2 || ends[0].Key != "b" || ends[1].Key != "c" {
		t.Errorf("Ends = %v", ends)
	}
	starts := database.Starts(edges)
...
func TestResolveNeedsARegistry(t *testing.T) {
	_, err := database.Resolve[proto.Message](context.Background(), bare{}, nil, nil)
	if err == nil {
...
func TestResolveRefusesAnUnknownResource(t *testing.T) {
	...
	_, err := database.Resolve[proto.Message](context.Background(), bare{}, reg,
		[]database.Ref{{Resource: "Ghost", Key: "x"}})
	if err == nil {
