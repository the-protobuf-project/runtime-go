...
const (
	// ORM is the relational driver, in the database/orm package.
	ORM Backend = "orm"
	// EVM is the Ethereum driver, in the blockchain/evm package.
	// Redis is the key-value driver, in the database/redis package.
	Redis Backend = "redis"

	// MongoDB is the document driver, in the database/mongodb package.
	MongoDB Backend = "mongodb"

	// ArangoDB is the document-and-graph driver, in the database/arangodb
	// package.
...
func Register(name Backend, opener Opener) {
	...
	if opener == nil {
		panic("database: Register opener is nil for backend " + string(name))
	}
	if _, dup := drivers[name]; dup {
		panic("database: Register called twice for backend " + string(name))
	}
	drivers[name] = opener
}
...
//
//	import _ "github.com/the-protobuf-project/runtime-go/database/orm"
//
//	drv, err := NewDriver(ORM, gormDB)
func NewDriver(name Backend, cfg any) (Driver, error) {
	...
	if !ok {
		return nil, fmt.Errorf(
			"database: backend %q is not registered (registered: %v); "+
