...
var (
	...
	ErrUnimplemented = errors.New("database: unimplemented")
	...
	// ErrBadConfig is returned by a driver's [Opener] when the configuration it
	// was handed through [NewDriver] is not the type that driver needs. It is a
	// wiring error rather than a storage outcome, so the adapter maps it to
	// gRPC Internal.
	ErrBadConfig = errors.New("database: invalid driver configuration")
)

// isNotFound reports whether an error means the record was not there, for the
// several places that skip one which vanished mid-operation.
