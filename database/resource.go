...
type Resource struct {
	...
	// IsEdge marks a resource that describes a connection between records rather
	// than a record.
	//
	// It matters because the two are stored differently on a graph backend — an
	// edge collection on ArangoDB, a relationship type on Neo4j — and because a
	// traversal that was not told which types to follow has to know which of the
	// registered resources are edges at all. A backend with no graph ignores it.
	IsEdge bool
