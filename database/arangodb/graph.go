...
var (
	_ store.Graph         = (*Driver)(nil)
	_ store.GraphMigrator = (*Driver)(nil)
)
...
func (d *Driver) Connect(ctx context.Context, edge *store.Resource, from, to store.Ref, props proto.Message) (store.Edge, error) {
	if edge == nil {
		return store.Edge{}, fmt.Errorf("arangodb: Connect needs an edge resource")
	}
	...
	if props != nil {
		cols, cerr := store.MessageToColumns(edge, props)
		if cerr != nil {
			return store.Edge{}, cerr
		}
	...
	if err != nil {
		return store.Edge{}, translate(err, edge, "")
	}
...
func (d *Driver) Disconnect(ctx context.Context, edge *store.Resource, key string) error {
	if edge == nil {
...
func (d *Driver) Neighbors(ctx context.Context, from store.Ref, opts store.TraverseOptions) ([]store.Edge, error) {
	o := opts
	if o.MaxDepth == 0 {
		o.MaxDepth = 1
	}
	...
	edges := make([]store.Edge, 0, len(paths))
	for _, p := range paths {
		if len(p.Edges) > 0 {
...
func (d *Driver) Traverse(ctx context.Context, from store.Ref, opts store.TraverseOptions) ([]store.Path, error) {
...
func (d *Driver) decodePath(vertices, edges []map[string]any, withProps bool) store.Path {
	path := store.Path{
		Vertices: make([]store.Ref, 0, len(vertices)),
...
func (d *Driver) resolveRef(ctx context.Context, ref store.Ref) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("%w: %s/%s", store.ErrNotFound, ref.Resource, ref.Key)
	}
...
// own rather than part of [store.Graph]. The edge collections are created if
...
// startup.
func (d *Driver) EnsureGraph(ctx context.Context, name string, defs []store.EdgeDefinition) error {
	if name == "" {
...
// [store.Migrator.DropSchema] on each collection, where it is visible in the
...
func aqlDirection(dir store.Direction) string {
	switch dir {
	case store.Inbound:
		return "INBOUND"
	case store.AnyDirection:
		return "ANY"
	default:
		return "OUTBOUND"
	}
