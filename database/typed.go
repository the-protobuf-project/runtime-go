...
func For[T proto.Message](d Driver, res *Resource) (Typed[T], error) {
	if d == nil {
		return Typed[T]{}, fmt.Errorf("database: For needs a driver")
	}
	if res == nil {
		return Typed[T]{}, fmt.Errorf("database: For needs a resource")
	}
	if res.New == nil {
		return Typed[T]{}, fmt.Errorf("database: resource %q has no New; it cannot allocate a message", res.Name)
	}
	if _, ok := res.New().(T); !ok {
		var zero T
		return Typed[T]{}, fmt.Errorf(
			"database: resource %q allocates %T, not %T — the descriptor and the type do not match",
...
func (t Typed[T]) Get(ctx context.Context, key string) (T, error) {
	...
	out, ok := msg.(T)
	if !ok {
		return zero, fmt.Errorf("database: %s returned %T, not %T", t.res.Name, msg, zero)
	}
	return out, nil
}
	...
	for i, msg := range res.Items {
		...
		if !ok {
			...
			return nil, "", 0, fmt.Errorf(
				"database: %s returned %T at index %d, not %T", t.res.Name, msg, i, zero)
		}
		out = append(out, item)
	}
...
func (t Typed[T]) All(ctx context.Context, opts ListOptions) ([]T, error) {
	...
	for {
		...
		if seen[next] {
			return nil, fmt.Errorf("database: %s repeated page token %q; refusing to loop", t.res.Name, next)
		}
		seen[next] = true
		opts.PageToken = next
	}
