...
func (s *stub) Get(context.Context, *database.Resource, string) (proto.Message, error) {
	s.gets.Add(1)
	if s.err != nil {
		return nil, s.err
	}
...
func (s *stub) List(context.Context, *database.Resource, database.ListOptions) (database.ListResult, error) {
	items := make([]proto.Message, s.items)
	return database.ListResult{Items: items, Total: int64(s.items)}, s.err
}

var userRes = &database.Resource{Name: "User", Table: "users", PKColumn: "id"}

func instrumented(d database.Driver, r *recorder) *database.DB {
	return database.Instrument(database.Build(d, "stub", "", nil), database.WithMeter(r))
}
...
func TestNotFoundIsNotAnError(t *testing.T) {
	r := newRecorder()
	db := instrumented(&stub{err: fmt.Errorf("%w: x", database.ErrNotFound)}, r)
...
func TestOutcomesAreDistinguished(t *testing.T) {
	for _, tc := range []struct {
		...
		{nil, "ok"},
		{fmt.Errorf("%w: x", database.ErrNotFound), "not_found"},
		{fmt.Errorf("%w: x", database.ErrAlreadyExists), "conflict"},
		{fmt.Errorf("%w: x", database.ErrUnimplemented), "unimplemented"},
...
func TestRowsReadIsCounted(t *testing.T) {
	...
	if _, err := db.List(context.Background(), userRes, database.ListOptions{}); err != nil {
		t.Fatal(err)
	}
...
func (b *batcherStub) CreateMany(context.Context, *database.Resource, []proto.Message) ([]database.WriteResult, error) {
	return nil, nil
}

func (b *batcherStub) GetMany(_ context.Context, _ *database.Resource, keys []string) ([]proto.Message, error) {
	b.gets.Add(1)
...
func TestInstrumentPreservesCapabilities(t *testing.T) {
	...
	if _, ok := withBulk.Driver.(database.Batcher); !ok {
	...
	if _, ok := withoutBulk.Driver.(database.Batcher); ok {
...
func TestInstrumentWithNothingIsAPassthrough(t *testing.T) {
	db := database.Build(&stub{}, "stub", "", nil)
	if database.Instrument(db) != db {
...
func TestResolveBatchesThroughBatcher(t *testing.T) {
	reg := database.NewRegistry(*userRes)
	refs := make([]database.Ref, 500)
	for i := range refs {
		refs[i] = database.Ref{Resource: "User", Key: fmt.Sprintf("u-%d", i)}
	}

	b := &batcherStub{}
	if _, err := database.Resolve[proto.Message](context.Background(), b, reg, refs); err != nil {
		t.Fatal(err)
	}
	...
	p := &stub{}
	if _, err := database.Resolve[proto.Message](context.Background(), p, reg, refs); err != nil {
		t.Fatal(err)
	}
