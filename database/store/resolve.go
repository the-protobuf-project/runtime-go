...
func Resolve[T proto.Message](ctx context.Context, d Driver, reg *Registry, refs []Ref) ([]T, error) {
	if d == nil {
		return nil, fmt.Errorf("store: Resolve needs a driver")
	}
	if reg == nil {
		return nil, fmt.Errorf("store: Resolve needs a registry to turn resource names into descriptors")
	}
	out := make([]T, len(refs))
	...
	for name, idx := range byResource {
		...
		if bulk {
			...
			for j, i := range idx {
				...
				if !ok {
					var zero T
					return nil, fmt.Errorf("store: Resolve: %s is %T, not %T", name, msgs[j], zero)
				}
				out[i] = typed
			}
			continue
		...
		for _, i := range idx {
			...
			if !ok {
				var zero T
				return nil, fmt.Errorf("store: Resolve: %s is %T, not %T", name, msg, zero)
			}
			out[i] = typed
		}
	}
