...
// every method on [store.Driver] already works against one — including
...
//
// What this package adds is [store.TimeSeries]: partitioning a table by time,
...
//	page, _ := db.Series.Range(ctx, res, store.RangeOptions{…})
//	buckets, _ := db.Series.Aggregate(ctx, res, store.AggregateOptions{…})
//
...
// [store.Resource.PKColumn] and remains the lookup key for Get, Update and
...
// useful and entirely TimescaleDB's own. Putting them in [store.TimeSeries]
