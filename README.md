...
| [`interfaces/orm`](interfaces/orm) | `github.com/the-protobuf-project/runtime-go/interfaces/orm` | Package orm implements the backend-agnostic store.Driver over GORM. |
| [`interfaces/store`](interfaces/store) | `github.com/the-protobuf-project/runtime-go/interfaces/store` | Package store is the backend-agnostic core of the runtime data layer: a single CRUD Driver contract that operates on proto messages, a Resource descriptor that mirrors protorm's IR at runtime, and a reflection bridge every driver reuses to move values between a proto message and a backend column map. |
