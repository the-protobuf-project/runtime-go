...
type telemetryCache struct {
	...
	total observability.UpDownCounter
}
...
func WithTelemetry(next Document, m observability.Meter) Document {
	if m == nil {
		m = observability.NoopMeter
	}
	return &telemetryCache{
		next: next,
		ops:  m.Counter("cache_operations_total", observability.WithUnit("1")),
		dur: m.Histogram("cache_operation_duration_seconds",
			observability.WithUnit("s")),
		hits:  m.Counter("cache_gets_total", observability.WithUnit("1")),
		total: m.UpDownCounter("cache_entries", observability.WithUnit("1")),
	}
}

// WithTelemetryMiddleware is [WithTelemetry] as a [Middleware], for use with
// [Chain].
func WithTelemetryMiddleware(m observability.Meter) Middleware {
...
func (t *telemetryCache) record(ctx context.Context, op string, start time.Time, err error) {
	...
	labels := observability.Labels{"operation": op, "outcome": outcome}
...
func (t *telemetryCache) Create(ctx context.Context, value any, opts ...Option) (string, error) {
	...
	if err == nil {
		t.total.Add(ctx, 1, observability.Labels{})
	}
	return out, err
}
...
func (t *telemetryCache) Get(ctx context.Context, id string, dest any) error {
	...
	if errors.Is(err, ErrNotFound) {
		t.hits.Add(ctx, 1, observability.Labels{"result": "miss"})
		t.record(ctx, "get", start, nil)
		return err
	}
	if err == nil {
		t.hits.Add(ctx, 1, observability.Labels{"result": "hit"})
	}
