...
// Driver is a database.Driver backed by EVM storage contracts. It encodes records
...
func (d *Driver) bound(ctx context.Context, res *database.Resource) (Contract, *bind.BoundContract, error) {
...
func (d *Driver) verifySchema(ctx context.Context, res *database.Resource, b *bind.BoundContract) error {
...
func (d *Driver) List(ctx context.Context, res *database.Resource, opts database.ListOptions) (database.ListResult, error) {
	...
	if err := b.Call(&bind.CallOpts{Context: ctx}, &out, "list"); err != nil {
		return database.ListResult{}, d.classify(err)
	}
	slice := reflect.ValueOf(out[0])
	if slice.Kind() != reflect.Slice {
		return database.ListResult{}, fmt.Errorf("evm: list() did not return an array")
	}
	...
	for i := offset; i < offset+limit; i++ {
		...
		msg, err := database.ColumnsToMessage(res, cols)
		if err != nil {
			return database.ListResult{}, err
		}
...
func (d *Driver) write(ctx context.Context, res *database.Resource, msg proto.Message, method string, onCreate bool) (database.WriteResult, error) {
	c, b, err := d.bound(ctx, res)
	if err != nil {
		return database.WriteResult{}, err
	}
	if d.cfg.Signer == nil {
		return database.WriteResult{}, errNoSigner
	}
	...
	if err != nil {
		return database.WriteResult{}, d.classify(err)
	}
	pending, err := d.await(ctx, tx)
	if err != nil {
		return database.WriteResult{}, err
	}
	wr := database.WriteResult{Message: msg, Pending: pending}
	if pending {
...
func page(opts database.ListOptions, total int) (offset, limit int) {
