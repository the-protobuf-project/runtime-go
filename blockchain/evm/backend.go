...
func open(cfg any) (store.Driver, error) {
	...
	case *Config:
		if c == nil {
			return nil, fmt.Errorf("%w: evm driver was given a nil *Config", store.ErrBadConfig)
		}
		return newChecked(*c)
	default:
		return nil, fmt.Errorf("%w: evm driver needs an evm.Config, got %T", store.ErrBadConfig, cfg)
	}
}
...
func newChecked(cfg Config) (store.Driver, error) {
	if cfg.Backend == nil {
		return nil, fmt.Errorf("%w: evm driver needs a chain Backend", store.ErrBadConfig)
	}
	if len(cfg.Resources) == 0 {
		return nil, fmt.Errorf("%w: evm driver needs at least one resource contract", store.ErrBadConfig)
	}
	return New(cfg), nil
}
