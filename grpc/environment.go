...
func applyEnvOverrides(opts *options.Options) {
	...
	shared.Telemetry().Logger.Debugf("applyEnvOverrides: scanning env with prefix %q", prefix)

	if host := os.Getenv(prefix + "_GRPC_HOST"); host != "" {
		shared.Telemetry().Logger.Debugf("applyEnvOverrides: %s_GRPC_HOST=%q overrides gRPC host", prefix, host)
		opts.GRPC.Host = host
	}
	if portStr := os.Getenv(prefix + "_GRPC_PORT"); portStr != "" {
		if port, err := strconv.Atoi(portStr); err == nil {
			shared.Telemetry().Logger.Debugf("applyEnvOverrides: %s_GRPC_PORT=%d overrides gRPC port", prefix, port)
			opts.GRPC.Port = port
		} else {
			shared.Telemetry().Logger.Debugf("applyEnvOverrides: %s_GRPC_PORT=%q is not a valid int, ignoring", prefix, portStr)
		}
	}
	if host := os.Getenv(prefix + "_HTTP_HOST"); host != "" {
		shared.Telemetry().Logger.Debugf("applyEnvOverrides: %s_HTTP_HOST=%q overrides HTTP host", prefix, host)
		opts.HTTP.Host = host
	}
	if portStr := os.Getenv(prefix + "_HTTP_PORT"); portStr != "" {
		if port, err := strconv.Atoi(portStr); err == nil {
			shared.Telemetry().Logger.Debugf("applyEnvOverrides: %s_HTTP_PORT=%d overrides HTTP port", prefix, port)
			opts.HTTP.Port = port
		} else {
			shared.Telemetry().Logger.Debugf("applyEnvOverrides: %s_HTTP_PORT=%q is not a valid int, ignoring", prefix, portStr)
		}
	}
	if host := os.Getenv(prefix + "_MCP_HOST"); host != "" {
		shared.Telemetry().Logger.Debugf("applyEnvOverrides: %s_MCP_HOST=%q overrides MCP host", prefix, host)
		opts.MCP.Host = host
	}
	if portStr := os.Getenv(prefix + "_MCP_PORT"); portStr != "" {
		if port, err := strconv.Atoi(portStr); err == nil {
			shared.Telemetry().Logger.Debugf("applyEnvOverrides: %s_MCP_PORT=%d overrides MCP port", prefix, port)
			opts.MCP.Port = port
		} else {
